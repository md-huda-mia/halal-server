...
const loginUser = async (req, res, next) => {
  try {
    ...
    return next(
      new HttpError("Login faild. Please check your credentials", 422)
    );
  }
};
  ...
  try {
    ...
    avatar.mv(
      ...
      async (err) => {
        ...
        if (!updateAvatar) {
          return next(new HttpError("Avater couldn't be changed.", 422));
        }
...
//     res.status(200).json(new HttpError(error));
//   } catch (error) {
//     return next(new HttpError(error));
//   }
// };
